...
if (point_distance(x, y, obj_player.x, obj_player.y) < 10 && state == "chase_player"){
	...
	if (state == "chase_player"){
		...
		last_rand = rand
		if (rand == 0){
			mp_potential_path(cop_path , obj_cop3_goal1.x, obj_cop3_goal1.y, 2,path_size, false)
			path_start(cop_path, 2, path_action_stop, true);
		}
		else if (rand == 1){
			mp_potential_path(cop_path , obj_cop3_goal2.x, obj_cop3_goal2.y, 2,path_size, false)
			path_start(cop_path, 2, path_action_stop, true);
		}
		else if (rand == 2){
			mp_potential_path(cop_path , obj_cop3_goal3.x, obj_cop3_goal3.y, 2,path_size, false)
			path_start(cop_path, 2, path_action_stop, true);
		}
